...
def parkanizer():
    ...
    try:
        ...
        wait.until(EC.title_is("Sign in | Share by Parkanizer"))
        wait.until(EC.url_contains("https://parkanizer-share.eu.auth0.com/login"))
        wait.until(
            EC.visibility_of_element_located((By.CLASS_NAME, "auth0-lock-submit"))
        )
        driver.find_element(By.NAME, "email").send_keys(parkanizer_user)
        driver.find_element(By.NAME, "password").send_keys(parkanizer_pass)
        driver.find_element(By.CLASS_NAME, "auth0-lock-submit").click()
